    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const payloadRecord =
            typeof itemRecord.payload === "object" && itemRecord.payload !== null
              ? (itemRecord.payload as Record<string, unknown>)
              : undefined;
          const key = `${nameKey || itemRecord.name || itemRecord.dataKey || "value"}`;
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const indicatorColor =
            color || payloadRecord?.fill || itemRecord.color || "currentColor";
                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <span className="text-muted-foreground">
                        {itemConfig?.label || String(itemRecord.name || "")}
                      </span>
                    </div>
                    {itemRecord.value !== undefined && itemRecord.value !== null && (
                      <span className="text-foreground font-mono font-medium tabular-nums">
                        {Number(itemRecord.value).toLocaleString()}
                      </span>
                    )}
                  </div>
                </>
          ...
          >
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: String(item.color || "transparent"),
                }}
              />
            )}
            {itemConfig?.label}
